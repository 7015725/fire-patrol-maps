...
### Added
- 新建标记待确认流程：点/矩形/多边形/圆形落笔后先显示虚线待确认草稿，不直接入库；草稿下方（底部空间不足时翻到上方）浮动取消 / 移动 / 编辑 / 确认四个图标按钮，按钮组随地图缩放同步放大缩小（0.8～1.75 倍钳制）且居中锚定不横飘。初始可拖动，移动为开关；形状可整体拖动 + 拉顶点/半径变形；编辑提前填类型 + 名称 + 备注暂存，确认落定再弹窗保存入库；弹窗内切换类型即按该类型独立从 001 起跳过占用自动重编。已有标记编辑方式不变，月检模式不受影响。
- 模式分区互斥：编辑（选择/绘制/待确认）与月检二选一；进月检强制回选择并清空进行中顶点与待确认草稿，隐藏绘制工具行与类型选择，标记锁定不可拖，点标记只做巡检打勾，退出回编辑；绘制工具下点中已有标记保留自动跳回选择并加状态文案说明。
## [0.10.1] — 2026-09-26
