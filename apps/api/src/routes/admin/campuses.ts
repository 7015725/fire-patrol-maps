...
async function canSwitchMode(
  ...
  if (to === "single_map") {
    if (nBuildings > 0) {
      return "存在楼宇时无法切换为单张地图模式，请先删除楼宇。";
    }
    if (directFloors.length > 1) {
      return "园区级楼层多于一个时无法切换为单张地图模式。";
    }
    if (buildingFloors.length > 0) {
      return "存在楼宇下的楼层时无法切换为单张地图模式。";
    }
  }

  if (to === "no_buildings") {
    if (nBuildings > 0) {
      return "存在楼宇时无法切换为「园区→楼层」模式，请先删除楼宇。";
    }
  }

  if (to === "full") {
    if (directFloors.length > 0) {
      return "存在园区级楼层时无法切换为完整层级模式，请先删除这些楼层。";
    }
  }
...
export function adminCampusesRoutes(getDb: () => Db) {
  ...
  app.post("/", async (c) => {
    ...
    const slug = resolveSlug(name, parsed.data.slug) ?? fallbackSlug("campus");
    if (!slug) {
    ...
    try {
      ...
      if (isUniqueViolation(err)) {
        return c.json({ error: "园区标识已存在" }, 409);
      }
      throw err;
    }
  });
