...
export function MapCanvas({
  ...
  // Native wheel listener with { passive: false }: React registers wheel
  // handlers as passive, so preventDefault() inside them is ignored — the
  // console fills with "Unable to preventDefault inside passive event
  // listener invocation" and the page scrolls while zooming.
  useEffect(() => {
    const el = viewportRef.current;
    if (!el) return;
